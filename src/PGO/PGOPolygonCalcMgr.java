...
public class PGOPolygonCalcMgr {
    ...
    // methods
    public static boolean isContained(Point test, ArrayList<PGOPolygon> polygons) {
    ...
    public static boolean checkContent(Point test, PGOPolygon polygon) {
    ...
    public static boolean isCrossed(Line2D.Double drawing,
    ...
    public static boolean isPointOnTheLine(Line2D.Double line,
            Point2D.Double pt) {
    ...
    public static boolean isIntersected(PGOPolygon curPolygon,
            ArrayList<PGOPolygon> polygons) {
        if (polygons.size() < 1)
            return false;
    ...
    public static boolean isOverlapped(PGOPolygon curPolygon,
            ArrayList<PGOPolygon> polygons) {
        for (PGOPolygon polygon : polygons) {
    ...
    public static Point isValidPt(Point oldPt, int width, int height) {
        Point newPt = oldPt;
    ...
    public static boolean areValidPolygons(ArrayList<PGOPolygon> polygons) {
        for (PGOPolygon polygon : polygons) {
    ...
    public static boolean isValidPolygon(PGOPolygon polygon) {
        ArrayList<Point> points = polygon.getPts();
    ...
    public static Point calcGravityPt(ArrayList<Point> pts) {
        Point pt1 = pts.get(0);
        Point pt2 = pts.get(1);
        Point pt3 = pts.get(2);
    ...
    public static Point findNearPt(Point pt, ArrayList<Point> fixedPts) {
        for (Point fixedPt : fixedPts) {
            if (pt.distance(fixedPt) < PGOPolygonCalcMgr.MAX_SNAP_DISTANCE) {
                pt.x = fixedPt.x;
                pt.y = fixedPt.y;
                break;
            }
